...
### 內部 tech-debt 候選（從 v2.11.7 cumulative review）

| # | 項目 | 估時 | 備註 |
|---|---|---|---|
| T1 | lib 抽出：`findComponentIndexByType` 集中（9 個 site at component/node/inspector/scene-tools） | 0.5 天 | 三方 review 必檢；savings ~35 lines |
| T2 | lib 抽出：dump-shape unwrap helper（16 sites）| 0.5 天 | savings ~45 lines |
| T3 | lib 抽出：scene root UUID extractor（14 sites）| 0.5 天 | savings ~30 lines |
| T4 | `new Promise(async (resolve) => …)` antipattern 清掃（19 sites at component/node/prefab/project/scene-view/server）| 1 天 | 純 cosmetic but 移除 silently swallow 風險 |
| T5 | get_node_tree / get_scene_hierarchy 加 maxDepth/maxNodes/summary cap | 1 天 | API 微調，需 changelog |
