...
**MCP 工具 workflow 缺塊（本 session 實機踩出，v2.1.4 必補）**：
- **`scene_create_scene` 沒 2D/3D 預設範本**：建出來的場景是空 root，要自己手
  build Camera+Canvas+UITransform+UI_2D layer 才能 preview。對應建議：
  schema 加 `template: 'empty' | '2d' | '3d'`，2d 預設組好 Canvas+Camera+
  UI_2D layer + cameraComponent ref。
- **`node_create_node` 沒 layer 參數，預設一律 DEFAULT (`1073741824`)**：
  Canvas 子節點 layer 該是 UI_2D (`33554432`) 才會被 UI camera 看到，否則
  preview 看不見任何東西。建議：`layer` 可 enum 字串
  (`"DEFAULT" | "UI_2D" | "UI_3D" | ...`) 或 number；parent 是 Canvas 後代
  時自動推 UI_2D。
- **`set_component_property` 對 component reference 的 propertyType 名稱不
  直觀**：`cameraComponent` 是 `cc.Camera` ref，要用 `propertyType: 'component'`
  + 「裝載該 component 的 node UUID」（不是 component UUID 本身）。我這
  session 第一次用 `propertyType: 'node'` 失敗時錯誤訊息是
  `Node reference value must be a string UUID`，指向錯方向。建議：
  `propertyType: 'node'` 收到 component ref 屬性時偵測並建議改用 `'component'`；
  描述加 example。
- **Sprite assign spriteFrame 後自動覆蓋 UITransform contentSize**：
  default `_sizeMode: 1 (TRIMMED)` → 自動取貼圖原生尺寸，先設的 contentSize
  被覆蓋。修法：要 `_sizeMode: 0 (CUSTOM)` 在 assign spriteFrame **前** 設、
  或 assign 之後重設 contentSize。建議：寫 helper / 工作流文件，或 assign
  spriteFrame 時若已有非預設 contentSize 自動切 CUSTOM。
- **`save_scene_as` timed out（>15s 才返回）**：本 session 用 Boot.scene 為底
  save_as 時 timed out 跑 background 才完成；後續直接 save 都 <2s。需要查
  是 stale-cache 還是真實作問題；很可能跟 facade saving 大場景有關。
...
v2.1.3 ✅ done（本 session）
   ...
   ├── _componentName workaround 砍掉        ✅（A/B 實機驗：preview-click fire OK）
   ├── setComponentProperty ladder dedup     ⏳ → v2.1.4
   └── MCPServer.getFilteredTools 二次過濾    ⏳ → v2.1.4
v2.1.4 ⏳ backlog（v2.1.3 剩餘 + 低優先項 B/C + P2-lite CRUD 合併 +
                   duplicatePrefab 收尾 + MCP 2D-scene workflow 補洞 5 條）
