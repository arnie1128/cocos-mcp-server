...
| 階段 | 主題 | 預估工時 | 風險 | 文件 |
|---|---|---|---|---|
| **P0** | 基線硬傷修復 | ~2 小時 | 低 | [`01-baseline-fixes.md`](./01-baseline-fixes.md) |
| **P1** | 架構債清理（換 SDK、去重複實例化、引入 zod） | ~5-7 天 | 中 | [`02-architecture.md`](./02-architecture.md) |
...
|---|---|
| 工具收斂 50 個 | P2（條件式執行） |
| 預製體 100% 對齊 | 不在 roadmap，視 P1/P2 做完後實機測試結果再決定 |
| 事件綁定補完 | 不在 roadmap，可單獨開 issue |
| 接口優化 | 散落在 P1（zod schema）與 P2 |
| 面板 UI | 不在 roadmap，可單獨開 issue |
