...
│   • http.createServer (手寫，T-P1-1 待換 @modelcontextprotocol/sdk)│
...
1. **手寫 HTTP server，尚未換官方 MCP SDK**：受限於 Cocos 編輯器內部 Node
   環境與 ESM 相容性（待驗證），原作者選擇手實作 JSON-RPC。代價是缺少
   capability negotiation、SSE、streaming。**P1 T-P1-1 將換成
   `@modelcontextprotocol/sdk` driven**，預估砍 ~200 行樣板，並一次取得
   protocol negotiation、streamable HTTP、structured content（T-P1-5）。
