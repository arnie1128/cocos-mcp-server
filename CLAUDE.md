...
`animation_get_animation_state_info`, `animation_set_animation_speed`,
`animation_check_animation_finished`; v2.9.4 added 2:
`debug_record_start` +
...
added 1: `debug_get_preview_mode`; v2.8.0 added 1: `debug_preview_control`;
...
15. **Gemini-compat: keep zod 4 + `target: 'draft-7'` for inline
    ...
    **We don't need that middleware** because `source/lib/schema.ts`
    ...
    copies, no `$ref`). All 181 v2.9.7 tool schemas are confirmed
    inline.
